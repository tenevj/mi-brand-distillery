                ...
                if (match) {
                    try {
                        ...
                        posts.push({
                            ...
                            content: content.replace(frontMatterRegex, '').trim(), // Remove the front matter from the content
                            image: `images/${frontMatter.image || 'default.jpg'}` // Default image if none specified
                        });
                    } catch (error) {
                        console.error("Error parsing YAML front matter in file:", file);
                        console.error(error);
...
function updateBlogPage() {
    ...
    const scopedStyles = `
        <style>
            ...
            .blog-posts-wrapper .post-image-container {
                max-height: 400px;
            }
            .blog-posts-wrapper .post-image-container img {
                max-height: 300px;
    ...
    if (!blogHtmlContent.includes('<style>.blog-posts-wrapper')) {
        blogHtmlContent = blogHtmlContent.replace(
            '</head>',
            `${scopedStyles}</head>` // Inject scoped styles
        );
    }

    // Generate blog posts HTML
    let postsHtml = blogPosts
        .map((post) => `
            <article class="blog-post">
                <div class="post-header">
                    <h2 class="post-title">${post.title}</h2>
            ...
            <hr class="post-divider">
        `)
        .join(''); // Join all post items into a single string

    postsHtml = `<div class="blog-posts-wrapper">${postsHtml}</div>`;

    // Inject blog posts into the designated area
    const postsContainerRegex = /<div id="blog-container">.*?<\/div>/s;
    if (postsContainerRegex.test(blogHtmlContent)) {
        blogHtmlContent = blogHtmlContent.replace(postsContainerRegex, `<div id="blog-container">${postsHtml}</div>`);
    } else {
        blogHtmlContent = blogHtmlContent.replace(
            '<div id="blog-container"></div>',
            `<div id="blog-container">${postsHtml}</div>`
        );
    }
